...
export default function Jobs() {
  ...
  const fetchAllJobs = async () => {
    try {
      ...
      const res = await api.get("/jobs/?category=project", { params });
  ...
  const fetchSavedJobs = async () => {
    try {
      setLoading(true);
      const res = await api.get("/my-jobs/saved/?category=project");
      setSavedJobs(
        res.data.map((item) => ({
          ...item.job,
          saved_id: item.id,
  ...
  const fetchAppliedJobs = async () => {
    try {
      setLoading(true);
      const res = await api.get("/my-jobs/applied/?category=project");
      setAppliedJobs(
        res.data.map((item) => ({
          ...item.job,
          applied_id: item.id,
  ...
  const fetchInterviewJobs = async () => {
    try {
      setLoading(true);
      const res = await api.get("/my-jobs/interview/?category=project");
      setInterviewJobs(
        res.data.map((item) => ({
          ...item.job,
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Collaborate on AI and tech projects
            </p>
          </div>
        </div>
