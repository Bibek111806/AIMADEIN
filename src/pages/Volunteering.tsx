...
export default function Jobs() {
  ...
  const fetchAllJobs = async () => {
    try {
      ...
      const res = await api.get("/jobs/?category=volunteering", { params });
  ...
  const fetchSavedJobs = async () => {
    try {
      setLoading(true);
      const res = await api.get("/my-jobs/saved/?category=volunteering");
      setSavedJobs(
        res.data.map((item) => ({
          ...item.job,
          saved_id: item.id,
  ...
  const fetchAppliedJobs = async () => {
    try {
      setLoading(true);
      const res = await api.get("/my-jobs/applied/?category=volunteering");
      setAppliedJobs(
        res.data.map((item) => ({
          ...item.job,
          applied_id: item.id,
  ...
  const fetchInterviewJobs = async () => {
    try {
      setLoading(true);
      const res = await api.get("/my-jobs/interview/?category=volunteering");
      setInterviewJobs(
        res.data.map((item) => ({
          ...item.job,
  ...
  const renderJobs = (jobList) => {
    if (loading) {
      return <div className="text-center text-gray-500">Loading volunteering...</div>;
    }

    if (jobList.length === 0) {
      return (
        <div className="text-gray-500 text-center py-4">No volunteering found.</div>
      );
    }
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Make a difference with your AI and tech skills
            </p>
          </div>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-4">
            <TabsTrigger value="all">All Volunteering</TabsTrigger>
